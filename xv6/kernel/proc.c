...
struct pqueue* gethighestque(void)
{
	if(p3.nproc)
	{
		for(int i = 0; i < p3.last; ++i)
		{
			if(p3.queue[i]->state == RUNNABLE)
			{
				return &p3;
			}
		}
	...
	if(p2.nproc)
	{
		for(int i = 0; i < p2.last; ++i)
		{
			if(p2.queue[i]->state == RUNNABLE)
			{
				return &p2;
			}
		}
...
void enqueue(struct pqueue *pq, struct proc *p)
{
	pq->queue[pq->last] = p;
	p->index = pq->last;
	++pq->last;
...
void dequeue(struct pqueue *pq, struct proc *p)
{
...
void drop(struct pqueue *pq, struct proc *p)
{
	int temp = p->prio;
	if (temp == 3)
	{
...
{
	struct pqueue *pq = gethighestque();
	for(int i =0; i < pq->last; ++i)
	{
...
void scheduler(void)
{
	...
	for(;;){
		...
		for(p = proc; p < &proc[NPROC]; p++) 
		{
			...
			struct pqueue *queue = gethighestque();

			// Loop through runnable until reached tick count
			while (p->state == RUNNABLE && p->ticks < queue->ntick)
			{
				if (p != gethighestproc())
				{
					break;
			...
			if(p->state == RUNNABLE && (p->prio == 3 || p->prio == 2 || p->prio == 1) && p->ticks >= queue->ntick)
			{	
				drop(queue, p);
				p->ticks = 0;
			}
			else if (p->state == RUNNABLE && p->prio == 0 && p->ticks >= p0.ntick)
			{
				p->ticks = 0;
				++p->full;
				dequeue(queue, p);
